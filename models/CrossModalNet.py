    ...
    def __init__(self, inchannels, args) -> None:
        ...
        for j in self.modalities:
            for k in self.modalities:
                if j == k: continue
                layers_name = j + k
                layers[layers_name] = TransformerEncoder(
                    inchannels,
                    num_heads=args['crossmodal_nheads'],
                    layers=args['num_crossmodal']
                )
            layers[f'mem_{j}'] = TransformerEncoder(
                inchannels * (n_modals - 1),
                num_heads=args['self_att_nheads'],
